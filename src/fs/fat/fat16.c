...
typedef unsigned FAT_ITEM_TYPE;
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    // Set to zero as it is created on the stack and therefore uninitialized
    ...
    while(true) 
    {
        ...
        {
            // Hit a blank record so we're done
            break;
        }

        // Is the directory entry free 
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    int total_sectors = root_dir_entries / disk->sector_size;
