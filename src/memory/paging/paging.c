...
{
    uint32_t* directory = kzalloc(sizeof(uint32_t) * PAGING_TOTAL_ENTRIES_PER_DIRECTORY);
    int offset = 0;
...
{
    int res = 0;
    if((uint32_t)virt % PAGING_PAGE_SIZE || (uint32_t)phys % PAGING_PAGE_SIZE || (uint32_t)phys_end % PAGING_PAGE_SIZE || (uint32_t)phys_end < (uint32_t)phys)
    {
        res = -EINVARG;
        goto out;
    }
